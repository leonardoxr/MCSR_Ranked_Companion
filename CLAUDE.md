...
MCSR Ranked Companion is a Next.js 15 + React 18 + TypeScript application that provides statistics, leaderboards, and live match viewing for MCSR (Minecraft Speedrun) Ranked players.
...
**Production URLs:**
- Primary: https://mcsr-ranked-companion.vercel.app
- Project URL: https://mcsr-ranked-companion-leonardoxrs-projects.vercel.app
- Git Branch URL: https://mcsr-ranked-companion-git-main-leonardoxrs-projects.vercel.app
...
# Link project to existing Vercel deployment
vercel link --yes
```
...
vercel inspect <url>             # Get detailed deployment info
...
**Configuration Notes:**
- The `output: 'standalone'` in [next.config.js](next.config.js) is for Docker compatibility and is ignored by Vercel
- Vercel auto-detects Next.js and configures build settings automatically
- Every push to `main` branch triggers a production deployment
- Pull requests get preview deployments automatically
- Build command: `npm run build` (includes type checking)
- Node.js version: 22.x (configured in Vercel project settings)
...
1. **API Client** ([src/lib/api/client.ts](src/lib/api/client.ts))
   ...
   - Base URL: `https://api.mcsrranked.com` (configurable via `NEXT_PUBLIC_MCSR_API_BASE_URL`)
   - Optional API key support via `NEXT_PUBLIC_MCSR_API_KEY`
