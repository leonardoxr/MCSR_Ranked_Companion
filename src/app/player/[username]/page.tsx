...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const playerUrl = `${SITE_URL}/player/${encodeURIComponent(username)}`;
...
function generatePlayerJsonLd(username: string) {
  return {
    ...
    mainEntity: {
      '@type': 'Person',
      name: username,
      url: `${SITE_URL}/player/${encodeURIComponent(username)}`,
    },
    breadcrumb: {
      '@type': 'BreadcrumbList',
      itemListElement: [
        ...
        },
        {
          ...
          position: 2,
          name: username,
          item: `${SITE_URL}/player/${encodeURIComponent(username)}`,
        },
      ],
    },
  };
